...
if Server then
    ...
    function DropPack:OnUpdate(deltaTime)
        ...
        if self.onGroundPoint and self.onGroundPoint ~= self:GetOrigin() and self.fallSpeed and weapon == nil then
            ...
            self.fallSpeed = math.max(0.00, self.fallSpeed + deltaTime)
            self:SetOrigin(self:GetOrigin() + Vector(0, -self.fallSpeed, 0))
			
			if self:GetOrigin().y < self.onGroundPoint.y then
				self:SetOrigin(self.onGroundPoint)
			end
